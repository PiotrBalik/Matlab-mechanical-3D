...
function OptimumDesign = myCostPSOdiscrete(UCs,UBs,x,params,Type)
% global numberElements elementNodes SectionType ll
...
% 	a=UBs(choice_column,13); % read
    ...
    % energy value
    e_st = e_st + (  .1*1000*mass*L); % material
...
if any(c >= 0 ) % if there is violation of constraints
	flag=1;
else
    flag=0;
end
% otherwise it's optimizing
switch rem(Type,10) % switch last digit
    case 1
        OptimumDesign = w+flag*1e6;
    case 2
        OptimumDesign = EE+flag*1e10;
    case 3
        OptimumDesign = [w EE]+flag*[1e6 1e10];
    otherwise
        OptimumDesign = 1;
end
