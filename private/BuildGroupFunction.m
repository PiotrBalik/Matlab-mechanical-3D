...
% TODO: make it self generate code and recompile itself - for speed!

%group function building
ngroup=length(cell2mat(Order)); segments=cell(1,ngroup);

% assume x=[A B C D], then x(1) is A at specific position and so on...
for id=1:length(Order)
    group=Order{id}; %current group indexes
    for position=1:length(group)
        %put group's number (id) in proper position defined in group
    	segments{group(position)}=sprintf('x(%d) ',id);
    end
end

textgroup=['group=@(x) [' cell2mat(segments) '];'];
